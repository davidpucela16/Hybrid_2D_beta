...
if Malphigui:
    ...
    csv_directory='/home/pdavid/Bureau/Updated_BCs_2/Figures_and_Tests/Double_source_6cells/csv_outputs'
else: #Auto_58
    ...
    csv_directory='/home/pdavid/Bureau/Code/Updated_BCs_2/Figures_and_Tests/Double_source_6cells/csv_outputs'
os.chdir(directory)
...
array_of_dist=np.squeeze(np.array(pd.read_csv(csv_directory + '/array_of_dist.csv')))
...
for d in array_of_dist:
    directory_COMSOL=directory_script + '/COMSOL_output/both_sources/d={}'.format(int(d))
...
for d in array_of_dist:
    directory_COMSOL=directory_script + '/COMSOL_output/source_sink/d={}'.format(int(d))
    pos_s=np.array([[0,-d/2-Rv[0]],[0,d/2+Rv[1]]])+np.array([[0.5,0.5],[0.5,0.5]])*L
    print("distance between sources= ", (np.linalg.norm(pos_s[1]-pos_s[0])-np.sum(Rv))/R_size)
    plot_sketch(x_coarse, y_coarse, directness, h_coarse, pos_s, L, directory_script)
